...
def parse_command_line():
    ...
    parser.add_argument(
        "-d", "--dryrun", action="store_true", help="Dont create or delete files"
    )
    parser.add_argument(
        "-f",
        "--forward",
    ...
    if args.forward == True:
        ...
        for trkdir in trkdirs:
            ...
            for gpxfile in files:
                ...
                if VERBOSE:
                    print(" INFO: processing {}".format(gpxfile))
    ...
    if args.reverse == True:
        ...
        for trkdir in trkdirs:
            ...
            for geofile in files:
                if VERBOSE:
                    print(" INFO: processing {}".format(geofile))
                gpxfile = pathlib.Path(
                    str(geofile).replace("2_geojson", "3_gpx")
                ).with_suffix(".gpx")
