...
def parseCommandLine():
    ...
    parser.add_argument('-t', '--tolerance', type=float,          help='When simplifying tracks use this tolerance in meters')
    parser.add_argument('-u', '--updown',    action='store_true', help='Split tracks into up/down hill segments')
...
if __name__ == '__main__':
    ...
    for fpath in fpaths:
        ...
        if VERBOSE : print(" ACTION: Converting waypoints to Feature Points")
        for wp in gpx.waypoints:
            if VERBOSE : print("  > WayPoint: {} -> ({},{}) type [{}]".format(wp.name,wp.latitude,wp.longitude,wp.description))
            geo_point = geojson.Point((wp.longitude,wp.latitude,wp.elevation), precision=args.precision)
            geo_features.append(geojson.Feature(properties={"name": wp.name,
                                                            "desc": wp.description},
                                                geometry=geo_point
                                                )
                                )
